...
here 
*/
SELECT s.season_id,home.team_id, SUM(f.home_points)
FROM fact_game f
JOIN dim_team AS home ON f.home_team_id = home.team_id
JOIN dim_team AS away ON f.visitor_team_id = away.team_id
JOIN dim_season s ON f.season_id = s.season_id
...
*/
SELECT f.game_id, d.year, home.team_id, away.team_id, f.home_points, f.visitor_points
FROM fact_game f
JOIN dim_team AS home ON f.home_team_id = home.team_id
JOIN dim_team AS away ON f.visitor_team_id = away.team_id
JOIN dim_date d ON f.date_id = d.date_id
...
*/
SELECT s.season_id, d.year, d.month, t.team_id, 
SUM(
	CASE 
	WHEN f.home_team_id = t.team_id THEN f.home_points
	WHEN f.visitor_team_id = t.team_id THEN f.visitor_points
...
/*
14- Compute the query to get the number of game (from game_fact) played by each player when his team play as home Team
*/
SELECT ps.player_name, COUNT(*)
FROM fact_game f
JOIN dim_team AS home ON f.home_team_id = home.team_id
JOIN dim_team AS away ON f.visitor_team_id = away.team_id
JOIN dim_season s ON f.season_id = s.season_id
JOIN dim_date d ON f.date_id = d.date_id
JOIN dim_ranking r ON r.team_id = home.team_id AND r.season_id = s.season_id
JOIN dim_player_dynamic pd ON pd.team_id = home.team_id AND pd.season_id = s.season_id
...
*/
SELECT f.game_id, home.abbreviation,f.home_points, f.visitor_points, pp.points, pp.assists
FROM fact_game f
JOIN dim_team AS home ON f.home_team_id = home.team_id
JOIN dim_team AS away ON f.visitor_team_id = away.team_id
JOIN dim_season s ON f.season_id = s.season_id
...
/*
16- Compute the query to GET the game where one of the player scored more than 50 points
*/
SELECT f.game_id, d.date, home.abbreviation, away.abbreviation
FROM fact_game f 
JOIN dim_team AS home ON f.home_team_id = home.team_id
JOIN dim_team AS away ON f.visitor_team_id = away.team_id
JOIN dim_season s ON f.season_id = s.season_id
